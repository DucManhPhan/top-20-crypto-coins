...
def lambda_handler(event, context):
    ...
    """
    print(f"[LAMBDA_HANDLER] Starting to pull crypto coins from CoinMarketCap at {datetime.now().isoformat()}")
    print(f"[LAMBDA_HANDLER] Environment variables - CMC_API_KEY_NAME: {CMC_API_KEY_NAME}, DYNAMODB_TABLE: {DYNAMODB_TABLE}")

    try:
        print(f"[LAMBDA_HANDLER] Step 1: Getting crypto coins from API")
        coins = get_top_crypto_coins()
        
        if not coins:
            print(f"[LAMBDA_HANDLER] No coins data received from API")
            return {
                'statusCode': 500,
                'body': json.dumps('Cannot get data from CoinMarketCap')
            }

        print(f"[LAMBDA_HANDLER] Step 2: Received {len(coins)} coins, saving to DynamoDB")
        save_to_dynamodb(coins)
        
        print(f"[LAMBDA_HANDLER] Step 3: Successfully completed processing")
        return {
            'statusCode': 200,
...
def get_api_key_from_cmc():
    ...
    """
    print(f"[GET_API_KEY] Getting API key from Secrets Manager: {CMC_API_KEY_NAME}")
    client = boto3.client('secretsmanager')
    try:
        response = client.get_secret_value(SecretId=CMC_API_KEY_NAME)
        secret = response['SecretString']
        print(f"[GET_API_KEY] Successfully retrieved API key")
        return secret
    except Exception as e:
...
def get_top_crypto_coins():
    ...
    """
    print(f"[GET_CRYPTO_COINS] Starting to get crypto coins data")
    ...
    try:
        ...
        print(f"[GET_CRYPTO_COINS] Making API request to CoinMarketCap")
        ...
        with urllib.request.urlopen(req) as response:
            ...
            if response.status != 200:
                print(f"[GET_CRYPTO_COINS] API Error: {data.get('status', {}).get('error_message', 'Unknown error')}")
                return []
...
def save_to_dynamodb(coins):
    ...
    """
    print(f"[SAVE_TO_DYNAMODB] Starting to save {len(coins)} coins to DynamoDB table: {DYNAMODB_TABLE}")
    ...
    try:
        ...
        for coin in coins:
            ...
            print(f"[SAVE_TO_DYNAMODB] Processing coin {saved_count + 1}/{len(coins)}: {coin_name} (ID: {coin_id})")
            ...
            saved_count += 1
            print(f"[SAVE_TO_DYNAMODB] Successfully saved {coin_name} (${quote.get('price', 'N/A')})")
